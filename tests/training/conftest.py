...
def node_embeddings(device, clip_model, mock_config):
    ...
    return NodeEmbeddings(config=mock_config, device=device, prepopulate_caches=False, clip_model=clip_model)
...
def mock_node_embeddings(mock_config):
    ...
    with patch.object(NodeEmbeddings, '_load_caches'):
        node_embeddings = NodeEmbeddings(config=mock_config, device="cpu", prepopulate_caches=False, clip_model=MagicMock())
