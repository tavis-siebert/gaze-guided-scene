...
class VideoMetadata:
    ...
    def __init__(self, config=None):
        ...
        self.num_object_classes = len(self.object_label_to_id)

        # Load video lengths for both train and val splits
        self.video_lengths = {}
        train_lengths = self.load_video_lengths(
            self.config.dataset.ego_topo.splits.train_video_lengths
        )
        val_lengths = self.load_video_lengths(
            self.config.dataset.ego_topo.splits.val_video_lengths
        )
        ...
        logger.info(
            f"Initialized metadata with {self.num_object_classes} object classes and {len(self.video_lengths)} videos"
        )
    ...
    def get_future_action_labels(
        self, video_name: str, current_frame: int
        ...
        try:
            with open(video_lengths_file) as f:
                for line in f.read().strip().split("\n"):
                    ...
                    vid_lengths[k] = int(v)
            logger.info(
                f"Loaded {len(vid_lengths)} video lengths from {video_lengths_file}"
            )
