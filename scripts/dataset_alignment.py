...
def load_action_mapping(mapping_csv: str) -> Dict[int, int]:
    ...
    return {int(row['egtea_action_id']): int(row['ego_topo_action_id']) 
            for _, row in mapping_df.iterrows()}


def build_suffix_map(ann_df: pd.DataFrame, action_mapping: Dict[int, int]) -> Dict[Tuple[int, ...], List[Dict[str, Any]]]:
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mapping-csv', required=True, help='Path to ego-topo-action-mapping.csv')
