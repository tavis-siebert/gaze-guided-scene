...
class YOLOWorldOnnxModel(YOLOWorldModel):
    ...
    def _load_model(self, model_path: Path, num_workers: Optional[int] = None) -> None:
        ...
        try:
            ...
            self.clip_model = ClipModel(
                name="ViT-B/32", device=self.text_embedding_device
            )
            ...
            logger.info(
                f"YOLO-World ONNX model loaded successfully on device: {self.device}"
            )
    ...
    def _run_inference(
        ...
        if not isinstance(image, Image.Image):
            raise TypeError(
                f"YOLOWorldOnnxModel.predict requires PIL.Image.Image, got {type(image)}"
            )
        ...
        input_img = (
            cv2.cvtColor(input_img, cv2.COLOR_BGR2RGB) / 255.0
        )  # To RGB and normalize
        input_img = input_img.transpose(2, 0, 1)  # HWC to CHW
        input_img = np.expand_dims(input_img, 0).astype(
            np.float32
        )  # Add batch dimension
