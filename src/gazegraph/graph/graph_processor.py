...
from gazegraph.graph.utils import filter_videos
...
def process_video(
    ...
    builder = GraphBuilder(
        config, split, enable_tracing=enable_tracing, output_dir=output_dir
    )
...
def build_graph(
    ...
    logger.info(
        f"Created graph checkpoints for {len(saved_paths)} videos in {split} split"
    )
    return saved_paths
...
def initialize_multiprocessing() -> None:
    ...
    try:
        ...
        logger.warning(
            "Multiprocessing start method already set, could not change to 'spawn'"
        )
...
def build_graphs(
    ...
    if not train_videos and not val_videos:
        logger.error(
            f"No videos matched the specified filters: {videos} in any split. Aborting."
        )
        return None
    ...
    if use_gpu and torch.cuda.is_available():
        ...
        if device_count < num_workers:
            logger.info(
                f"Number of workers ({num_workers}) exceeds available GPUs ({device_count})"
            )
            logger.info("Multiple workers will share GPUs")
    else:
        use_gpu = False
    ...
    logger.info(
        f"Created {len(all_paths['train'])} train checkpoints and {len(all_paths['val'])} val checkpoints"
    )
