...
class TestScoreComponents:
    ...
    def test_from_dict_partial(self):
        data = {"confidence": 0.8, "stability": 0.7}
...
class TestDetection:
    def test_to_dict(self):
        components = ScoreComponents(confidence=0.8, stability=0.7, gaze_proximity=0.9)
...
class TestObjectDetector:
    def test_init(self, mock_detector, mock_config):
        assert (
            mock_detector.min_fixation_frame_ratio
            == mock_config.graph.fixated_object_detection.min_fixation_frame_ratio
        )
        assert (
            mock_detector.bbox_stability_weight
            == mock_config.graph.fixated_object_detection.weights.bbox_stability
        )
        assert (
            mock_detector.gaze_proximity_weight
            == mock_config.graph.fixated_object_detection.weights.gaze_proximity
        )
        assert (
            mock_detector.confidence_weight
            == mock_config.graph.fixated_object_detection.weights.confidence
        )
        assert (
            mock_detector.duration_weight
            == mock_config.graph.fixated_object_detection.weights.duration
        )
        assert (
            mock_detector.bbox_stability_threshold
            == mock_config.graph.fixated_object_detection.thresholds.bbox_stability
        )
        assert (
            mock_detector.gaze_proximity_threshold
            == mock_config.graph.fixated_object_detection.thresholds.gaze_proximity
        )
        assert (
            mock_detector.confidence_threshold
            == mock_config.graph.fixated_object_detection.thresholds.confidence
        )
    ...
    def test_compute_mean_gaze_distance(self, mock_detector):
        ...
        assert (
            abs(
                mock_detector._compute_mean_gaze_distance(bboxes, gaze_points)
                - expected_mean
            )
            < 0.1
        )
        ...
        assert mock_detector._compute_mean_gaze_distance([], gaze_points) == float(
            "inf"
        )
    ...
    def test_compute_fixation_scores(self, mock_detector):
        ...
        with (
            patch.object(ObjectDetector, "_compute_mean_iou", return_value=0.7),
            patch.object(ObjectDetector, "_compute_geometric_mean", return_value=0.8),
            patch.object(
                ObjectDetector, "_compute_mean_gaze_distance", return_value=10.0
            ),
        ):
            ...
            try:
                ...
                assert (
                    cup_scores["duration_weighted"] == 0.8
                )  # mean_confidence * fixation_ratio (0.8 * 1.0)
                ...
                assert (
                    mock_detector.fixation_scores["bowl"] == bowl_scores["final_score"]
                )
    ...
    def test_compute_fixation_scores_with_filtering(self, mock_detector):
        ...
        with (
            patch.object(ObjectDetector, "_compute_mean_iou") as mock_iou,
            ...
            assert (
                mock_detector.filtered_stats["confidence"] == 2
            )  # Both bowl and spoon fail here
            assert (
                mock_detector.filtered_stats["stability"] == 0
            )  # None get filtered here because of confidence filtering
            assert (
                mock_detector.filtered_stats["gaze_proximity"] == 0
            )  # None get filtered here
    ...
    def test_detect_objects(
        self, mock_compute_scores, mock_perform_detection, mock_detector
    ):
        # Setup mocks
        frame = torch.zeros(3, 100, 100)
    ...
    def test_has_fixated_objects_compute_scores(self, mock_detector):
        ...
        with patch.object(
            ObjectDetector,
            "_compute_fixation_scores",
            wraps=mock_detector._compute_fixation_scores,
        ) as spy:
