...
class DummyMetadata:
    def __init__(self, valid_frames=None, start=0, end=9):
        self.valid_frames = valid_frames if valid_frames is not None else list(range(start, end+1))
        self.start = start
        self.end = end
...
def test_sampling_matrix(strategy, oversampling, allow_duplicates, samples_per_video):
    ...
    samples = sampling.get_samples(
        checkpoints, 'video', strategy, samples_per_video, allow_duplicates, oversampling, metadata
    )
    # Check output types
    ...
    if oversampling:
        ...
        potential = [(cp, cp.get_future_action_labels(cp.frame_number, metadata)) for cp in checkpoints]
    ...
    if strategy == "all" or samples_per_video == 0:
        ...
        if not allow_duplicates:
            # No duplicates
            assert len({(x[0].frame_number, x[1]['label']) for x in samples}) == samples_per_video
...
def test_invalid_strategy():
    ...
    with pytest.raises(ValueError):
        sampling.get_samples(checkpoints, 'video', 'not_a_strategy', 5, False, False, metadata)
    with pytest.raises(ValueError):
        sampling.get_samples(checkpoints, 'video', 'not_a_strategy', 5, False, True, metadata)
...
def test_oversampling_fallback_on_metadata():
    ...
    samples = sampling.get_samples(checkpoints, 'video', 'all', 0, False, True, metadata)
