...
class YOLOWorldUltralyticsModel(YOLOWorldModel):
    ...
    def _load_custom_model(self, class_names: List[str]) -> None:
        ...
        if self.custom_model_path.exists():
            logger.info(
                f"Loading custom YOLO-World model from: {self.custom_model_path}"
            )
            self.model = YOLOWorld(str(self.custom_model_path))
            self.names = class_names
            logger.info("Custom YOLO-World model loaded successfully")
        else:
            logger.info(
                f"Custom model not found at {self.custom_model_path}, will save after setting classes"
            )
    ...
    def _load_model(self, model_path: Path) -> None:
        ...
        try:
            ...
            logger.info("YOLO-World Ultralytics model loaded successfully")
    ...
    def _run_inference(
        ...
        if not isinstance(image, Image.Image):
            raise TypeError(
                f"YOLOWorldUltralyticsModel.predict requires PIL.Image.Image, got {type(image)}"
            )
