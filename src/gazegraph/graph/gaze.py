...
from typing import List, Tuple, Optional, Union, Any, Iterator, Dict
...
class GazeType(IntEnum):
    ...
    SMOOTHED_FIXATION = 11  # For saccades reclassified as fixations based on spatial stability
...
class GazeProcessor:
    ...
    def __init__(self, config: DotDict, gaze_data: np.ndarray):
        ...
        self.spatial_window_size = getattr(config.graph, 'spatial_window_size', 5)
        self.spatial_distance_threshold = getattr(config.graph, 'spatial_distance_threshold', 0.05)
        self.reclassify_saccades = getattr(config.graph, 'reclassify_saccades', True)
    ...
    def _preprocess_gaze_data(self) -> None:
        ...
        for i in range(len(processed_data)):
            ...
            valid_points = window[~np.isin(window[:, 2], [GazeType.UNTRACKED, GazeType.TRUNCATED])]
            if len(valid_points) < 3:  # Need at least 3 points for meaningful stability analysis
                continue
            ...
            if (median_distance < self.spatial_distance_threshold and 
                int(processed_data[i, 2]) == GazeType.SACCADE and
                self.reclassify_saccades):
        ...
        for i in range(1, len(processed_data) - 1):
            if (int(processed_data[i, 2]) == GazeType.SACCADE and
                (int(processed_data[i-1, 2]) in (GazeType.FIXATION, GazeType.SMOOTHED_FIXATION)) and
                (int(processed_data[i+1, 2]) in (GazeType.FIXATION, GazeType.SMOOTHED_FIXATION))):
    ...
    def _is_spatially_stable(self, current_pos: Tuple[float, float]) -> bool:
        ...
        valid_positions = [(x, y) for x, y, t in self.gaze_buffer 
