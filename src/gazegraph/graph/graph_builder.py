...
class GraphBuilder:
    ...
    def process_video(
        self, video_name: str, print_graph: bool = False
    ) -> Optional[str]:
        ...
        self.tracer = GraphTracer(
            self.config.directories.traces, video_name, enabled=self.enable_tracing
        )
        ...
        if saved_path:
            logger.info(
                f"Saved {len(self.checkpoint_manager.checkpoints)} checkpoints to {saved_path}"
            )
            return saved_path
    ...
    def _initialize_components(self):
        ...
        self.first_frame, self.last_frame = (
            self.video.first_frame,
            self.video.last_frame,
        )
        logger.info(
            f"Processing frames from {self.first_frame} to {self.last_frame} based on action annotations"
        )

        # Create scene graph
        self.scene_graph = Graph(video_length=self.video.length)
    ...
    def _process_frame(
        self, frame: torch.Tensor, gaze_point: GazePoint, is_black_frame: bool
    ) -> None:
        """
        ...
        if self.frame_num > self.last_frame:
            raise StopProcessingException(
                reason=f"Reached end of annotated frames (frame {self.last_frame})"
            )

        node_id = (
            self.scene_graph.current_node.id
            if self.scene_graph.current_node.id >= 0
            else None
        )

        if gaze_point is not None:
            self.tracer.log_frame(
                self.frame_num, gaze_point.position, int(gaze_point.type), node_id
            )
        else:
        ...
        self.checkpoint_manager.checkpoint_if_needed(
            frame_num=self.frame_num, non_black_frame_count=self.non_black_frame_count
        )
    ...
    def _handle_saccade(self, gaze_point: GazePoint) -> None:
        ...
        logger.debug(
            f"- Current gaze position (normalized): ({gaze_point.position[0]:.2f}, {gaze_point.position[1]:.2f})"
        )

        # Only proceed if we found fixated objects during this fixation period
        if not self.object_detector.has_fixated_objects():
            logger.debug(
                "- No fixated objects found during this fixation period, skipping node creation"
            )
        ...
        logger.debug(
            f"- Fixated object: {fixated_object} (confidence: {confidence:.2f})"
        )
        ...
        prev_position = (
            self.prev_gaze_point.position if self.prev_gaze_point else (0, 0)
        )
    ...
    def _finish_final_fixation(self) -> None:
        ...
        last_position = (
            self.prev_gaze_point.position if self.prev_gaze_point else (0.5, 0.5)
        )
