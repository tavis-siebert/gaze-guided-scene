...
def get_deterministic_jitter(
    node_id: Any, scale: float = LAYOUT_JITTER_SCALE
) -> Tuple[float, float]:
...
def initialize_positions_from_angles(G: nx.DiGraph) -> Dict:
    ...
    for node in G.nodes():
        if any(
            "angle_degrees" in G[node][target].get("features", {})
...
def compute_graph_layout(G: nx.DiGraph) -> Dict[Any, Tuple[float, float]]:
    ...
    initial_pos = (
        initialize_positions_from_angles(G) if len(G.nodes) < MAX_ANGLE_NODES else None
    )
