...
class VideoDisplay(BaseComponent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.frame_cache = {}
        self.max_cache_size = (
            max_cache_size // batch_size
        )  # Convert to number of batches
        ...
        self.verbs = (
            self._load_mapping_file(self.verb_idx_file) if self.verb_idx_file else {}
        )
        self.nouns = (
            self._load_mapping_file(self.noun_idx_file) if self.noun_idx_file else {}
        )
            ...
            for file_path in [self.train_split_file, self.val_split_file]:
                if file_path and os.path.exists(file_path):
                    df = pd.read_csv(
                        file_path,
                        header=None,
                        names=["name", "start", "end", "label1", "label2"],
                        sep="\t",
                    )
    ...
    def get_frame(self, frame_number: int) -> Optional[np.ndarray]:
        ...
        with self.video_lock:
            ...
            for _ in range(batch_start, batch_end):
                ...
                if not success:
                    logger.warning(
                        f"Failed to read frame at position {batch_start + len(batch_frames)}"
                    )
                    continue
    ...
    def _get_gaze_traces(
        self, events: List, traces: List[go.Trace], fig: go.Figure
    ) -> None:
        ...
        for event in events:
            if event.event_type == "frame_processed":
                ...
                gaze_info = GAZE_TYPE_INFO.get(
                    gaze_type, {"color": "black", "label": f"Other ({gaze_type})"}
                )
                ...
                event.event_type == "edge_added"
                and event.data["edge_type"] == "saccade"
            ):
    ...
    def _get_detection_traces(
        self, playback: Playback, frame_number: int, traces: List[go.Trace]
    ) -> None:
        ...
        current_label = detection_event.data.get(
            "current_detected_label", fixated_object
        )
        ...
        hover_text = self._create_detection_hover_text(
            current_label, fixated_object, potential_labels
        )
    ...
    def _create_detection_hover_text(
        self, current_label: str, fixated_object: str, potential_labels: dict
    ) -> str:
        ...
        sorted_labels = sorted(
            potential_labels.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _get_yolo_detection_traces(
        ...
        for detection in detections:
            ...
            hover_text = self._create_yolo_hover_text(
                class_name,
                score,
                is_fixated,
                is_top_scoring,
    ...
    def get_figure(self, frame_number: int) -> go.Figure:
        ...
        traces = []
        events = (
            self.playback.get_events_for_frame(frame_number) if self.playback else []
        )
        self._get_gaze_traces(events, traces, fig)
        self._get_detection_traces(
            self.playback, frame_number, traces
        ) if self.playback else None
        self._get_yolo_detection_traces(
            self.playback, frame_number, traces
        ) if self.playback else None
    ...
    def _add_action_overlay(self, frame_number: int, traces: List[go.Trace]) -> None:
        ...
        # Group actions by their verb_id and noun_id for display purposes
        action_counts = {}
        action_details = {}
        ...
        # Create formatted action text list with counts for display
        action_texts = []
        ...
        # Create individual hover texts for each original action
        hover_texts = []
        for action in actions:
            action_text = f"{action['verb']} {action['noun']}"
            hover_text = (
