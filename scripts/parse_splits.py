...
def parse_split_file(
    file_path: str,
    verb_mapping: Dict[int, str],
    action_mapping: Dict[int, str],
    noun_mapping: Dict[int, str],
) -> List[dict]:
    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            clip_name, start_time, end_time, start_frame, end_frame = (
                clip_match.groups()
            )
            ...
            noun_names = [
                noun_mapping.get(n_id, f"Unknown noun {n_id}") for n_id in noun_ids
            ]
