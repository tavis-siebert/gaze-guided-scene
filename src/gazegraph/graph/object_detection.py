...
class Detection:
    ...
    @classmethod
    def from_dict(
        cls, data: Dict[str, Any], frame_idx: Optional[int] = None
    ) -> "Detection":
        ...
        bbox = (
            tuple(det_data["bbox"])
            if isinstance(det_data["bbox"], list)
            else det_data["bbox"]
        )

        # Use provided frame_idx if available, otherwise from data
        actual_frame_idx = (
            frame_idx if frame_idx is not None else det_data.get("frame_idx", -1)
        )
...
class ObjectDetector:
    ...
    def __init__(
        ...
        self.min_fixation_frame_threshold = (
            config.graph.fixated_object_detection.min_fixation_frame_threshold
        )
        self.min_fixation_frame_ratio = (
            config.graph.fixated_object_detection.min_fixation_frame_ratio
        )
        self.bbox_stability_weight = (
            config.graph.fixated_object_detection.weights.bbox_stability
        )
        self.gaze_proximity_weight = (
            config.graph.fixated_object_detection.weights.gaze_proximity
        )
        self.confidence_weight = (
            config.graph.fixated_object_detection.weights.confidence
        )
        self.duration_weight = config.graph.fixated_object_detection.weights.duration

        # Thresholds for component scores
        self.bbox_stability_threshold = (
            config.graph.fixated_object_detection.thresholds.bbox_stability
        )
        self.gaze_proximity_threshold = (
            config.graph.fixated_object_detection.thresholds.gaze_proximity
        )
    ...
    def detect_objects(
        self, frame: torch.Tensor, gaze_point: GazePoint, frame_idx: int
    ) -> List[Detection]:
        ...
        try:
            ...
            if found_fixated_objects:
                ...
                if len(self.all_detections) > 0:
                    ...
                    top_score_class = (
                        max(scores.keys(), key=lambda x: scores[x]["final_score"])
                        if scores
                        else None
                    )
                    ...
                    for detection in detections:
                        if detection.is_fixated and detection.class_name in scores:
                            ...
                            detection.is_top_scoring = (
                                detection.class_name == top_score_class
                            )
                            ...
                            detection.components.gaze_proximity = obj_scores[
                                "gaze_proximity"
                            ]
                            detection.components.fixation_ratio = obj_scores[
                                "fixation_ratio"
                            ]
                            detection.components.duration_weighted = obj_scores[
                                "duration_weighted"
                            ]
                            detection.components.gaze_distance = obj_scores[
                                "gaze_distance"
                            ]
    ...
    def _compute_fixation_scores(self) -> Dict[str, Dict[str, float]]:
        ...
        total_frames_any_object_fixated = len(
            set(d.frame_idx for d in self.all_detections if d.is_fixated)
        )
        ...
        for obj_name in unique_objects:
            ...
            obj_detections = [
                ...
                if d.class_name == obj_name and d.is_fixated
            ]
            ...
            if fixation_frames < self.min_fixation_frame_threshold:
                logger.debug(
                    f"Object {obj_name} filtered out: fixation frames {fixation_frames} < threshold {self.min_fixation_frame_threshold}"
                )
                continue
            ...
            if fixation_ratio < self.min_fixation_frame_ratio:
                logger.debug(
                    f"Object {obj_name} filtered out: fixation ratio {fixation_ratio:.2f} < threshold {self.min_fixation_frame_ratio}"
                )
            ...
            relevant_gaze_points = [
                g for g in self.gaze_points if g[2] in obj_frame_indices
            ]
            ...
            if mean_confidence < self.confidence_threshold:
                logger.debug(
                    f"Object {obj_name} filtered out: confidence score {mean_confidence:.2f} < threshold {self.confidence_threshold}"
                )
            ...
            if stability_score < self.bbox_stability_threshold:
                logger.debug(
                    f"Object {obj_name} filtered out: stability score {stability_score:.2f} < threshold {self.bbox_stability_threshold}"
                )
                self.filtered_stats["stability"] += 1
                continue

            # 3. Compute gaze proximity weighting
            gaze_distance = self._compute_mean_gaze_distance(
                bboxes, relevant_gaze_points
            )
            ...
            if gaze_weight < self.gaze_proximity_threshold:
                logger.debug(
                    f"Object {obj_name} filtered out: gaze proximity {gaze_weight:.2f} < threshold {self.gaze_proximity_threshold}"
                )
            ...
            logger.debug(
                f"  - Duration score: {duration_weighted_score:.4f} (ratio: {fixation_ratio:.2f})"
            )
            logger.debug(
                f"  - Confidence score: {mean_confidence:.4f} (threshold: {self.confidence_threshold})"
            )
            logger.debug(
                f"  - Stability score: {stability_score:.4f} (threshold: {self.bbox_stability_threshold})"
            )
            logger.debug(
                f"  - Gaze weight: {gaze_weight:.4f} (distance to center: {gaze_distance:.2f}, threshold: {self.gaze_proximity_threshold})"
            )

        # Log filter statistics
        if self.filtered_stats["total_considered"] > 0:
            logger.debug(
                f"Fixation filtering stats: {self.filtered_stats['passed_all']}/{self.filtered_stats['total_considered']} passed all filters"
            )
            logger.debug(
                f"  - Filtered by fixation ratio: {self.filtered_stats['fixation_ratio']}"
            )
            logger.debug(
                f"  - Filtered by confidence: {self.filtered_stats['confidence']}"
            )
            logger.debug(
                f"  - Filtered by stability: {self.filtered_stats['stability']}"
            )
            logger.debug(
                f"  - Filtered by gaze proximity: {self.filtered_stats['gaze_proximity']}"
            )
    ...
    def _perform_detection(
        self, frame: torch.Tensor, gaze_x: int, gaze_y: int, frame_idx: int
    ) -> List[Detection]:
        ...
        for detection in raw_detections:
            ...
            is_fixated = (
                left - margin <= gaze_x <= left + width + margin
                and top - margin <= gaze_y <= top + height + margin
            )
    ...
    def _log_fixated_objects(self, frame_idx: int, detections: List[Detection]) -> None:
        ...
        if fixated_detections:
            ...
            if top_object:
                logger.info(
                    f"[Frame {frame_idx}] Top fixated object: {top_object.class_name} (score: {top_object.fixation_score:.2f})"
                )
            else:
                logger.info(
                    f"[Frame {frame_idx}] Found {fixated_count} fixated objects"
                )

            logger.debug(
                f"[Frame {frame_idx}] {fixated_count} fixated object detections:"
            )
            for detection in fixated_detections:
                ...
                logger.debug(
                    f"  - {detection.class_name} ({score_info}, "
                ...
                if detection.fixation_score > 0:
                    ...
                    logger.debug(
                        f"    Components: conf={comp.confidence:.2f}, "
    ...
    def _compute_gaze_distance(
        self, bbox: Tuple[int, int, int, int], gaze: Tuple[int, int]
    ) -> float:
    ...
    def _compute_mean_gaze_distance(
        ...
        frame_to_bbox = {
            frame_idx: bbox for bbox, (_, _, frame_idx) in zip(bboxes, gaze_points)
        }
        ...
        for gaze_x, gaze_y, frame_idx in gaze_points:
            if frame_idx in frame_to_bbox:
                distance = self._compute_gaze_distance(
                    frame_to_bbox[frame_idx], (gaze_x, gaze_y)
                )
    ...
    def get_fixated_object(self) -> Tuple[str, float]:
        ...
        if not self.fixation_scores:
            ...
            logger.warning(
                f"  - Total objects considered: {self.filtered_stats['total_considered']}"
            )
            logger.warning(
                f"  - Filtered by fixation ratio: {self.filtered_stats['fixation_ratio']}"
            )
        ...
        for obj, s in sorted(
            self.fixation_scores.items(), key=lambda x: x[1], reverse=True
        ):
