...
class Video:
    ...
    def __init__(self, video_name: str, config=None):
        ...
        # Load paths
        self.video_path = (
            Path(self.config.dataset.egtea.raw_videos) / f"{video_name}.mp4"
        )
        ...
        self.first_frame, self.last_frame = self.metadata.get_action_frame_range(
            video_name
        )
        self.length = self.metadata.get_video_length(video_name)

        logger.info(
            f"Loaded video '{video_name}' with {len(self.records)} records, "
    ...
    def seek_to_frame(self, frame_num: int):
        ...
        try:
            ...
            self.gaze_processor = GazeProcessor(
                self.config, parse_gtea_gaze(str(self.gaze_path))
            )
            self._gaze_iter = iter(self.gaze_processor)
            ...
            logger.debug(
                f"Seeked video and gaze data for {self.video_name} to frame {frame_num}"
            )
