...
def extend_action_mapping(
    ...
    with (
        ...
        extended_header = (
            header[:2]
            + ["verb_name", "noun_name", "action_description", "egtea_action_id"]
            + header[2:]
        )
        ...
        for row_idx, row in enumerate(reader, 1):
            if len(row) >= 4:  # Ensure row has expected format
                ...
                extended_row = (
                    row[:2]
                    + [verb_name, noun_name, action_description, str(egtea_action_id)]
                    + row[2:]
                )
                writer.writerow(extended_row)

    # Print matching statistics
    print("Action matching statistics:")
    ...
    print(
        f"  - Total processed: {matching_stats['exact_matches'] + matching_stats['verb_noun_matches'] + matching_stats['flexible_matches'] + matching_stats['unmatched']}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Extend action mapping CSV with verb and noun names"
    )
    parser.add_argument(
        "--mapping",
    ...
    parser.add_argument(
        "--verb-idx",
        default="egtea_gaze/action_annotation/verb_idx.txt",
        help="Path to the verb index file",
    )
    parser.add_argument(
        "--noun-idx",
        default="egtea_gaze/action_annotation/noun_idx.txt",
        help="Path to the noun index file",
    )
    parser.add_argument(
        "--action-idx",
        default="egtea_gaze/action_annotation/action_idx.txt",
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Save debug information about matching"
    )
