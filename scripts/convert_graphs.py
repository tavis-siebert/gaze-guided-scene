...
def convert_checkpoint_file(input_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        reduction_percent = (
            0
            if not checkpoints
            else ((len(checkpoints) - len(pruned_checkpoints)) / len(checkpoints)) * 100
        )
        ...
        logger.info(
            f"Reduced checkpoints from {len(checkpoints)} to {len(pruned_checkpoints)} ({reduction_percent:.1f}% reduction)"
        )
        return True
...
def convert_all_graphs(
    ...
    successful = 0
    stats = {"total_original_checkpoints": 0, "total_pruned_checkpoints": 0}
            ...
            try:
                with torch.serialization.safe_globals([GraphCheckpoint]):
                    ...
                    if isinstance(pruned_data, dict) and "checkpoints" in pruned_data:
                        stats["total_pruned_checkpoints"] += len(
                            pruned_data["checkpoints"]
                        )
            except Exception:
                pass
...
def main():
    ...
    parser.add_argument(
        "--input-dir",
        type=str,
        default="datasets/graphs",
        help="Directory containing old format checkpoint files",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="datasets/graphs_pruned",
    ...
    if stats["total_original_checkpoints"] > 0:
        reduction = (
            (stats["total_original_checkpoints"] - stats["total_pruned_checkpoints"])
            / stats["total_original_checkpoints"]
        ) * 100
        ...
        logger.info(
            f"Storage savings: Removed {stats['total_original_checkpoints'] - stats['total_pruned_checkpoints']} redundant checkpoints"
        )
