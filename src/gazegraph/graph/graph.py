...
class Graph:
    ...
    def add_node(self, label: str, visit: VisitRecord) -> Node:
        ...
        """
        node = Node.create(node_id=self.num_nodes, label=label, visit=visit)
    ...
    def update(
        ...
        if matching_node:
            ...
            logger.debug(
                f"- Updated node features: visits={total_visits}, total_frames={total_frames}"
            )
        else:
            # Create new node
            ...
            logger.info(
                f"New node {next_node.id} created for object '{fixated_object}'"
            )

        # Add edge if needed (node changed and no existing connection)
        if next_node != self.current_node and not self.has_neighbor(
            self.current_node.id, next_node.id
        ):
            forward_edge, backward_edge = self.add_edge(
                self.current_node, next_node, prev_gaze_pos, curr_gaze_pos, num_bins
            )
            ...
            if forward_edge and prev_node_id >= 0:
                ...
                logger.info(
                    f"Edge added from node {prev_node_id} to node {next_node.id}"
                )
    ...
    def _find_matching_node(self, label: str) -> Optional[Node]:
        ...
        """
        return Node.find_matching(graph=self, curr_node=self.current_node, label=label)
