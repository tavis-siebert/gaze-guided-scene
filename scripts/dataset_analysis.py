...
def collect_noun_distribution(
    df: pd.DataFrame, noun_mapping: Dict[int, str]
) -> Dict[str, Counter]:
    """
...
def collect_action_distribution(
    df: pd.DataFrame, action_mapping: Dict[int, str]
) -> Dict[str, Counter]:
    """
...
def plot_edge_to_node_ratio(df: pd.DataFrame, output_path: str):
    ...
    df["edge_to_node_ratio"] = df["num_edges"] / df["num_nodes"].replace(
        0, 1
...
def plot_bidirectional_stats(df: pd.DataFrame, output_path: str):
    ...
    bidirectional_counts = (
        df.groupby(["split", "bidirectional"]).size().unstack(fill_value=0)
    )
...
def plot_future_actions_count(df: pd.DataFrame, output_path: str):
    ...
    for i, split in enumerate(["train", "val"]):
        ...
        if not split_df.empty:
            plt.subplot(1, 2, i + 1)
            sns.histplot(
                split_df["future_action_count"], discrete=True, kde=False, color=f"C{i}"
            )
...
def analyze_dataset(
    df: pd.DataFrame, noun_mapping_path: str, action_mapping_path: str, output_dir: str
):
    """
    ...
    plot_edge_node_distribution(
        df, os.path.join(output_dir, "node_edge_distribution.png")
    )

    plot_edge_to_node_ratio(df, os.path.join(output_dir, "edge_node_ratio.png"))

    plot_bidirectional_stats(df, os.path.join(output_dir, "bidirectional_stats.png"))

    plot_future_actions_count(df, os.path.join(output_dir, "future_actions_count.png"))
    ...
    with open(os.path.join(output_dir, "summary_stats.txt"), "w") as f:
        ...
        f.write(
            f"  Graphs with some non-bidirectional edges: {len(df) - df['bidirectional'].sum()}\n\n"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Analyze dataset CSV and generate visualizations"
    )
    parser.add_argument("csv_path", help="Path to the dataset CSV file")
    parser.add_argument(
        "-o", "--output-dir", help="Output directory for analysis results"
    )
    parser.add_argument(
        "--noun-mapping",
    ...
    output_dir = args.output_dir or os.path.join(
        os.path.dirname(args.csv_path), "analysis"
    )
