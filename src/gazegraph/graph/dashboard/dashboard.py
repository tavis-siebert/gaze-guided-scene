...
class Dashboard:
    ...
    def _create_layout(self) -> dbc.Container:
        ...
        return dbc.Container(
            [
                dcc.Store(
                    id="play-state", data={"is_playing": False, "last_update": 0}
                ),
                dcc.Interval(
                    id="auto-advance",
                ...
                dbc.Row(
                    [
                        dbc.Col([self.playback_controls.create_layout()], width=12),
                    ],
                    className="mb-2",
                ),
                dbc.Row(dbc.Col(self.meta_info.create_layout()), className="mb-2"),
    ...
    def _register_callbacks(self, app: dash.Dash) -> None:
        ...
        @app.callback(
            [
                ...
                Output(
                    f"{self.playback_controls.component_id}-current-time", "children"
                ),
            ],
            [
    ...
    def run(
        self, port: int = 8050, debug: bool = False, use_reloader: bool = False
    ) -> None:
