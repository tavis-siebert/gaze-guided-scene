    ...
    for split in ['train', 'val']:
        ...
        for i, (x, edge_index, edge_attr, y) in enumerate(zip(x_list, edge_index_list, edge_attr_list, y_list)):
            ...
            if "future_actions" in y:
                ...
                future_actions = [idx + 1 for idx in torch.nonzero(y["future_actions"]).squeeze(1).tolist()]
            ...
            if "future_actions_ordered" in y:
                # Convert to 1-indexed
                future_actions_ordered = [idx + 1 for idx in y["future_actions_ordered"].tolist()]
                future_actions_ordered_str = ",".join(str(idx) for idx in future_actions_ordered)
                future_actions_ordered_names = "|".join(action_mapping.get(idx, f"Unknown-{idx}") for idx in future_actions_ordered)
