                    ...
                    'actions': [
                        {
                            ...
                            'verb_name': action_map[action['action_key']]['verb_name'],
                            'noun_name': action_map[action['action_key']]['noun_name'],
                            'description': action_map[action['action_key']]['action_description'],
...
def main():
    parser = argparse.ArgumentParser(description='Find videos with specific action suffix')
    parser.add_argument('action_ids', type=int, nargs='+',
                        help='List of action IDs to match as suffix')
    ...
    suffix_length = args.suffix_length if args.suffix_length is not None else len(args.action_ids)
    ...
    try:
        ...
        print(f"Error: {e}")
        print(f"Make sure the paths to the mapping and train files are correct.")
        sys.exit(1)
