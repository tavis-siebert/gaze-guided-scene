...
def test_initialization(backend):
    ...
    model = YOLOWorldModel.create(
        backend=backend, conf_threshold=0.35, iou_threshold=0.7, device="cpu"
    )
...
def test_object_detection_suite(yolo_world_model, test_dir):
    ...
    for img_file in test_dir.glob("*.png"):
        ...
        print(
            f"Backend: {yolo_world_model._get_backend_name()}, Image: {img_file.name}"
        )
        print(f"Expected: {expected_objects}")
        print(
            f"Detected: {[f'{d["class_name"]} ({d["score"]:.3f})' for d in detections]}"
        )
...
def test_custom_model_save_load(custom_classes, ultralytics_model_path):
    ...
    assert custom_model_path.exists(), (
        f"Custom model file not found at {custom_model_path}"
    )
    # Load the custom model in a new instance
    new_model = YOLOWorldUltralyticsModel(
        use_custom_model=True, custom_classes=custom_classes
    )
    assert new_model.names == custom_classes, (
        "Loaded custom model does not have the expected classes"
    )


@pytest.mark.gpu
def test_custom_model_confidence_improvement(
    yolo_world_model, test_image, custom_classes
):
    pytest.skip(
        "Custom model does not seem to have any (positive) effect on accuracy or speed"
    )
    ...
    # Load the custom model in a new instance
    loaded_custom_model = YOLOWorldUltralyticsModel(
        use_custom_model=True, custom_classes=custom_classes
    )
    ...
    assert custom_avg_conf > standard_avg_conf, (
        f"Custom model confidence ({custom_avg_conf}) not higher than standard ({standard_avg_conf})"
    )
