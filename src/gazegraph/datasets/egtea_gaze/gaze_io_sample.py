...
def _str2frame(frame_str, fps=None):
    ...
    time_sec = (
        3600 * int(splited_time[0]) + 60 * int(splited_time[1]) + int(splited_time[2])
    )
...
def parse_gtea_gaze(filename, gaze_resolution=None):
    ...
    for frame_idx in range(0, num_frames + 1):
        ...
        if (px < 0 or px > (gaze_resolution[1] - 1)) or (
            py < 0 or py > (gaze_resolution[0] - 1)
        ):
...
if __name__ == "__main__":
    ...
    logger.info(
        "Frame {:d}, Gaze Point ({:02f}, {:0.2f}), Gaze Type: {:s}".format(
            1000,
            test_data_01[1000, 0],
            test_data_01[1000, 1],
