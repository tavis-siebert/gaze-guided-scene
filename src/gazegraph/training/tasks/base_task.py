...
class BaseTask:
    def __init__(
        ...
        self.optimizer = get_optimizer(
            config.training.optimizer, self.model, config.training.lr
        )
    ...
    def _setup_tensorboard_writer(self):
        ...
        run_numbers = [
            int(d.split("_")[1]) for d in run_dirs if d.split("_")[1].isdigit()
        ]
    ...
    def _setup_data(self):
        ...
        if not graphs_dir.exists():
            self.logger.error(
                f"Graphs directory {graphs_dir} not found. Run 'python main.py build' first."
            )
