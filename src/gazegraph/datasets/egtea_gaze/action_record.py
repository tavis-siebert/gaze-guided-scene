...
class ActionRecord:
    ...
    def _load_name_mappings(cls, base_dir: Optional[str] = None) -> None:
        ...
        logger.info(
            f"Loaded {len(cls._verb_id_to_name)} verbs and {len(cls._noun_id_to_name)} nouns"
        )

    @classmethod
    def _load_all_records(
        cls,
    ) -> Tuple[Dict[str, List["ActionRecord"]], List["ActionRecord"]]:
        """
        ...
        try:
            for split_name in ("train", "val"):
                ...
                try:
                    ...
                    logger.error(
                        f"Error loading action records for {split_name} split: {e}"
                    )
                    raise
        finally:
        ...
        logger.info(
            f"Loaded {len(train_records)} training records and {len(records_by_video)} videos"
        )
        return records_by_video, train_records

    @classmethod
    def _compute_action_mapping(
        cls, records: List["ActionRecord"], num_classes: int = NUM_ACTION_CLASSES
    ...
    @classmethod
    def _compute_and_set_action_mapping(
        cls, train_records: List["ActionRecord"], num_classes: int = NUM_ACTION_CLASSES
    ) -> None:
        ...
        if not train_records:
            raise ValueError(
                "Cannot initialize action mapping: no training records provided"
            )
        ...
        if not cls._verb_id_to_name or not cls._noun_id_to_name:
            raise RuntimeError(
                "Name mappings must be loaded before initializing action mapping"
            )
        ...
        logger.info(
            f"Initialized action mapping with {len(cls._action_to_idx)} actions "
    ...
    def get_past_action_records(cls, video_name: str, current_frame: int) -> list:
        ...
        return sorted(
            [
                r
                for r in records
                if r.start_frame <= current_frame and r.action_idx is not None
            ],
    ...
    @classmethod
    def create_future_action_labels(
        cls,
        video_name: str,
        current_frame: int,
        ...
        future_actions = [
            ...
            if record.action_idx is not None
        ]
            ...
            with open(filepath, "r") as f:
                for line_num, line in enumerate(f, 1):
                    if line.strip():
                        ...
                        if (
                            len(row) < 4
                        ):  # Need at least video_name, start, end, and one label
                            raise ValueError(
                                f"Invalid record format at line {line_num}"
                            )
    ...
    def __str__(self) -> str:
        ...
        action = (
            f"{self.verb_name} {self.noun_name}"
            if self.verb_name and self.noun_name
            else f"({self.verb_id}, {self.noun_id})"
        )
