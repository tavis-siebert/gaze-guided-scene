...
from gazegraph.graph.node import Node
...
def get_roi(
    image: torch.Tensor, roi_center: Tuple[int, int], roi_size: int
...
def filter_videos(
    video_list: List[str], filter_names: Optional[List[str]], logger
) -> List[str]:
    ...
    filtered_videos = [
        vid for vid in video_list if any(name in vid for name in filter_names)
    ]

    if not filtered_videos:
        logger.warning(
            f"No videos matched the specified filters: {filter_names} in this split"
        )
        return []
    ...
    for filter_name, matches in matches_by_filter.items():
        ...
        logger.info(
            f"Filter '{filter_name}' matched {match_count} video(s) in this split"
        )
        if match_count <= 5:  # Show specific matches only if there aren't too many
            logger.info(f"  Matched: {', '.join(matches)}")

    logger.info(
        f"Filtered {len(video_list)} videos down to {len(filtered_videos)} based on specified names"
    )
...
class GraphTraversal:
    ...
    @staticmethod
    def dfs(
        graph: "Graph", start_node_id: int, visited: Optional[Set[int]] = None
    ) -> Set[Node]:
        """
    ...
    @staticmethod
    def get_all_nodes(
        graph: "Graph", start_node_id: int = -1, mode: str = "dfs"
    ) -> List[Node]:
        """
...
class FeatureMatcher:
    ...
    def compute_homography(
        kp1: List, kp2: List, matches: List, ransac_threshold: float = 7.0
