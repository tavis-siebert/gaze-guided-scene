...
class Playback:
    ...
    def _rotate_event_angles(self, event: GraphEvent) -> None:
        ...
        if "features" in event.data and "angle" in event.data["features"]:
            event.data["features"]["angle"] = self._rotate_angle(
                event.data["features"]["angle"], True
            )
            if "angle_degrees" in event.data["features"]:
                event.data["features"]["angle_degrees"] = self._rotate_angle(
                    event.data["features"]["angle_degrees"], False
                )
    ...
    def _process_event(self, event: GraphEvent) -> None:
        ...
        if event.event_type == "node_added":
            ...
            self.graph.add_node(node_id, label=label, features=features)
            ...
            if node_id in self.graph:
                self.graph.nodes[node_id].update(label=label, features=features)
                self.last_updated_node = node_id
                self.most_recent_node = node_id
            ...
            self.graph.add_edge(
                source_id, target_id, edge_type=edge_type, features=features
            )
